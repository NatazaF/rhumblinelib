...
impl Rhumb {
    pub fn rhumbline_inverse(lat1: f64, lon1: f64, lat2: f64, lon2: f64) -> (f64, f64) {
        let mut distance: f64 = 1.0;
        let mut azimuth: f64 = 1.0;
        unsafe {
            rhumb_inverse(
                lat1,
                lon1,
...
mod tests {
    ...
    fn test_rhumbline_inverse() {
        ...
        let (distance, az) = Rhumb::rhumbline_inverse(lat1, lon1, lat2, lon2);

        assert_eq!(distance.round() as u32, 10000);
        assert_eq!(az.round() as u32, 45);
    }
    ...
    fn test_rhumbline_direct() {
        ...
        let az = 45.0;
        let dist = 10000.0;
        let (out_latitude, out_longitude, _out_azimuth) =
            Rhumb::rhumbline_direct(lat, lon, az, dist);
